...
class ClientOperations:
    ...
    def sign_up(client: Client) -> int:
        verify_if_email_is_in_use_filter_query = { 
            "email": client.email 
        }  
        ...
        if result:
            ...
            client_data_dict = client_data.__dict__
            result = Connection.insert_user_collection(client_data_dict)
            if result:
                return Http.created
    ...
    def move_card_personal(user_id: str, card_data: Any, card_current_column: str, card_destiny_column:str):
        ...
        add_card_result = Connection.update_user_collection(
            move_card_personal_filter_query, 
            add_card_to_destiny_column_query
        )

        if add_card_result:
            delete_card_result = Connection.update_user_collection(
                move_card_personal_filter_query, 
                delete_card_from_current_column_query
            )
    ...
    def delete_card_personal(user_id: str, card_id: str, card_column: str) -> int:
        ...
        delete_card_personal_filter_query = { 
            "_id": client_id 
        }
    ...
    @validate_arguments
    @staticmethod
    def change_personal_card_content(user_id: str, card_column: str, card_id: str, new_card_content: str) -> int:
