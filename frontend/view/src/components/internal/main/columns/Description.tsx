...
export default function Description({ 
    ...
    async function handleChangePersonalColumnColor(
        currentColorValue: string
    ): Promise<void> {

        const status = await changeGroupColumnColor(
            currentColorValue, 
            currentGroupId, 
            area
        );
    ...
    async function handleChangeGroupColumnColor(
        currentColorValue: string
    ): Promise<void> {

        const status = await changePersonalColumnColor(
            currentColorValue, 
            area
        );
    ...
    return (
        ...
        >
            ...
            <input 
                ...
                onFocus={(event) => updateInitialColor(event)} 
                onBlur={(event) => updateFinalColor(event)}
            />
        </div>
    );
}
