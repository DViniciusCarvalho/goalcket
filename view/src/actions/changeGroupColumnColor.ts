...
Promise<number> {
    const requestConfig = getChangeColorGroupRequestConfig(currentColorValue, currentGroupId, column);
    const promisedResponseData = doChangeGroupColumnColorRequest(requestConfig);

    return promisedResponseData;
}

function getChangeColorGroupRequestConfig(currentColorValue: string, currentGroupId: string, column: string):
